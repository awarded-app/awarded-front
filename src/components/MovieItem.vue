    ...
    <main>
      ...
      <ul v-if="isFull">
        <li v-for="nomination in nominations" :key="nomination.id" class="flex">
          <p class="mr-2">
            <star :winner="true" class="mr-1 text-sm" />{{
              nomination.category.name
            }}
          </p>
          <ul class="text-gray-500 flex">
            <li
              v-for="(nominatedPerson, index) in nomination.nominatedPeople
                .nodes"
              :key="nominatedPerson.id"
            >
              {{ nominatedPerson.person.name
              }}<span v-if="nominatedPerson.character">
      ...
      <ul v-if="isCategory" class="text-gray-500 flex">
        <li
          v-for="(nominatedPerson, index) in nominations[0].nominatedPeople
            .nodes"
          :key="nominatedPerson.id"
        >
          {{ nominatedPerson.person.name
            ...
            (as {{ nominatedPerson.character }})</span
          ><span v-if="index < nominations[0].nominatedPeople.nodes.length - 1"
            >,&nbsp;
          </span>
        </li>
      </ul>
