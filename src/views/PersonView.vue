...
export default {
  ...
  apollo: {
    ...
    nominatedPeople: {
      query() {
        return gql`
          query ${this.awardType}NominatedPeople($condition: ${
          this.AwardType
        }NominatedPersonCondition) {
            ${this.awardType}NominatedPeople(condition: $condition) {
              totalCount
              nodes {
      ...
      update(data) {
        return (this.nominations = data[`${this.awardType}NominatedPeople`].nodes.map(node => {
          return {
            ...node.nomination,
            prizes: { ...node[`${this.awardType}NominatedPersonPrizes`] }
          };
        }));
      }
    }
